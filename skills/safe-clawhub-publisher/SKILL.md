...
9. Ask for explicit confirmation immediately before publication. A request to prepare, validate, dry-run, install, or log in is not permission to publish. Confirmation applies only to the displayed account, target, version, command, file set, and fingerprint.
10. Run the pre-publication integrity gate below, then publish once. Never automatically issue a second publish command. If the outcome is unclear, follow the fixed verification and retry budget in [references/troubleshooting.md](references/troubleshooting.md): at most two non-mutating remote checks, then stop and hand control back to the user. A version missing from CLI/API inspection can reflect indexing delay, cache, or rate limiting and does not by itself prove publication failed.
11. Verify the remote version, metadata, file hashes, tags, changelog, owner, and security status. For an unclear publish response, check immediately and at most once more after the server's `Retry-After` delay, capped at two minutes. When machine-readable inspection remains unavailable or stale, give the user the canonical package page `https://clawhub.ai/<owner>/skills/<slug>` and stop automated work. Record user-visible confirmation separately from CLI/API verification.
...
- Use a publication attempt budget: one initial attempt and no automatic retries. A second attempt requires authoritative failure/rejection evidence plus fresh confirmation for the exact retry command. Do not make more than one confirmed retry; after another failure or ambiguous outcome, stop for manual resolution.
